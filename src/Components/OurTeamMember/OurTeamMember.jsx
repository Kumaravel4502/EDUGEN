          ...
          <div className="relative w-[90%] mx-auto bg-[#FFF7FA] pt-20 pb-12 sm:pt-24 sm:pb-14 md:pt-28 md:pb-16 lg:pt-32 lg:pb-18 overflow-hidden rounded-xl mb-12">
                <div className="relative z-20 text-center px-4">
                    ...
                    <p className="mt-3 text-sm sm:text-base text-gray-700 font-medium">
                        ...
                        <span className="text-[#8A47CB] font-semibold"> – Our Team </span>
                    </p>
                </div>
        ...
        <div className="w-[90%] mx-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-10">
            ...
            <div className="space-y-5">
              <p className="sub-title text-[19px] font-semibold">Team Members</p>
              ...
              <button className="relative rounded-sm h-[55px] w-[200px] mt-6 mb-1 bg-white px-3 text-black cursor-pointer transition-all before:absolute before:bottom-0 before:left-0 before:top-0 before:z-1 before:h-full before:w-4 before:rounded-[2px] before:bg-black before:transition-all before:duration-500 hover:text-white hover:before:w-full hover:before:rounded-sm">
                <span className="relative z-10 flex justify-between items-center font-semibold ml-1">
                  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="2em" height="2em" className="pt-[4px]">
            ...
            {/* Team Members */}
            {teamData.map((item) => (
              <div key={item.id} className="bg-white p-6 border-3 border-dashed border-purple-500 rounded-lg shadow-md transition-transform duration-300 hover:scale-105 w-full mx-auto">
                {/* Image and Basic Info - Clickable */}
                <Link to={`/our-team/${item.id}`} className="block">
                  <div className="flex justify-center">
                    <img src={item.image} alt={item.name} className="w-52 h-52 p-1 transition-transform duration-500 hover:scale-110 cursor-pointer" />
                  </div>

                  {/* Details */}
                  <div className="text-center mt-4">
                    <h3 className="text-lg font-semibold">{item.name}</h3>
                    <p className="text-purple-600 text-sm font-medium">{item.position}</p>
                    <p className="text-gray-600 mt-2 text-[17px] font-semibold">{item.designation}</p>
                  </div>
                </Link>

                {/* Social Media Links */}
                <div className="flex justify-center gap-6 items-center mt-5">
                  <a href={item.google} target="_blank" rel="noopener noreferrer" className="faq-button butt hover:-translate-y-2 duration-700">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="3em" height="3em">
                      <path fill="none" stroke="#fff" strokeWidth="1.5" d="M13.365 2.83a9.25 9.25 0 0 1 4.744 2.089c.338.284.336.794.024 1.106l-1.616 1.616c-.312.312-.816.306-1.171.044a5.365 5.365 0 1 0 1.615 6.705h-3.91a.8.8 0 0 1-.8-.8V11.3a.8.8 0 0 1 .8-.8h7.493c.316 0 .61.186.681.495c.313 1.362-.125 3.246-.158 3.384l-.004.016c-.528 1.963-1.661 3.706-3.274 4.944a9.25 9.25 0 1 1-4.424-16.51" />
                    </svg>
                  </a>
                  <a href={item.github} target="_blank" rel="noopener noreferrer" className="faq-button butt hover:-translate-y-2 duration-700">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 128 128" width="5em" height="5em">
                  ...
                  <a href={item.linkedin} target="_blank" rel="noopener noreferrer" className="faq-button butt hover:-translate-y-2 duration-700">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="5em" height="5em">
                      <g>
